...
class D2PAgent: 
    ...
    def replay(self,samples,batch_size):
      ...
      loss_for_prior = np.zeros((batch_size, 1)) 
      
      for i in range(0,batch_size):
        state = samples["obs"][i,:]
        action = int(samples["acts"][i])
        reward = samples["rews"][i]
        new_state = samples["next_obs"][i,:]
        done= samples["done"][i]
        ...
        if done:
          ...
          loss_for_prior[i, 0] =  (targets[i, action]-(reward + Q_future * self.gamma))**2
          targets[i,action] = reward + Q_future * self.gamma
      with tf.device("/TPU:0"):
        loss = self.model.train_on_batch(inputs, targets)
        self.loss = loss
      return loss_for_prior
